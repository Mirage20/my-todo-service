...
# A service representing a network-accessible REST API
service http:Service /todos on new http:Listener(7080) {
    ...
    resource function get .(boolean? done) returns Todo[] {
        lock {
            Todo[] todos;
            if done is boolean {
    ...
    resource function get [int id]() returns Todo? {
        lock {
    ...
    resource function post .(@http:Payload CreateTodoInput todoInput) returns Todo {
        lock {
    ...
    # + updatedTodo - Updated todo item.
    # + return - updated Todo item.
    resource function put [int id](@http:Payload Todo updatedTodo) returns Todo? {
        lock {
